...
bool Mesh::Hit(const ray::ray_t& ray, float minDistance, float maxDistance, ray::raycastHit_t& raycastHit)
{
	// check cast ray with mesh triangles 
	for (int i = 0; i < m_vertices.size(); i += 3)
	{
		ray::Triangle triangle( m_vertices[i], m_vertices[i + 1], m_vertices[i + 2] , m_material);
...
void Mesh::ProcessMesh(aiMesh* mesh, const aiScene* scene, const glm::mat4& transform)
{
	...
	for (unsigned int i = 0; i < mesh->mNumVertices; i++)
	{
		...
		positions.push_back(position); //<add position to positions vector>
	}
